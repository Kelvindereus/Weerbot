...
async def weather_NL_general(m, result_city):
        ...
        if response_api_json['cod'] == "404":
            ...
            await m.channel.send(response_api_json, "Default")
...
async def weather_NL_city(m, city_user_input, server_id_from_m):
        ...
        if response_api_json['cod'] == "404":
            ...
            embedVar = discord.Embed(title="Weer-info voor de stad: " +str (city_user_input), description="Info geleverd door Openweathermap. Default plan!", color=color)
...
async def weather_NL_city_tomorrow(m, city_user_input, weather_time_user_input):
    ...
    if "morgen" in weather_time_user_input:
            ...
            ##gemiddelde embed
                    embedVar = discord.Embed(title="Weer-info voor morgen voor de stad: " +str (city_user_input), description="Info geleverd door Openweathermap. Default plan!", color=color)
                    ...
                    embedVar.add_field(name="Gemiddelde gevoeltemperatuur ", value=aft + str (round(averagefeeltempdag1,1)) + str ("℃"), inline=False)
...
async def send_help_embed(m):
    embedVar = discord.Embed(title="WeerBot help menu! (Standaard versie.)", description="Voor de community, door de community!", color=0x00ff00)
